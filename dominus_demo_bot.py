...
Changelog v4.36 — Finanzmathematische Optimierungen (8 Fixes):
  F1: FIX 4 — calc_tp_price() rechnet jetzt Taker-Fee ein (TAKER_FEE=0.0006).
      TP-Trigger werden erhoeht sodass der NETTO-ROI nach Gebühren dem Ziel
      entspricht. Bei 10x/0.06%: +1.2% Aufschlag auf alle TP-Preise.
      Konfigurierbar via Railway-Variable TAKER_FEE.
  F2: FIX 5 — Funding Rate Warnung bei extrem negativen Werten.
      Nach jedem DCA-TP-Update wird die aktuelle Funding Rate abgefragt.
      Warnung per Telegram wenn Rate < FUNDING_WARN_THRESHOLD (-0.05%/8h).
  F3: FIX 6 — TP-Ausführung als Limit-Order statt Market (optional).
      USE_LIMIT_TP=true (Railway) setzt executePrice mit LIMIT_TP_BUFFER
      (0.1%) unter dem Trigger. Weniger Slippage, aber kein 100% Fill-Garantie.
  F4: FIX 9 — Timestamp-Lücke im Main-Loop geschlossen.
      fetch_time wird VOR get_recent_fills_all() gesetzt. Fills die während
      setup_new_trade/update_tp_for_position eintreffen gehen nicht mehr verloren.
  F5: FIX 10 — TP-Verifikation nach Platzierung.
      verify_tp_orders() prüft 3s nach place_tp_orders() ob die erwartete
      Anzahl DOM_-eigener TPs wirklich auf der Exchange aktiv ist. Telegram-
      Alarm wenn TPs fehlen.
  F6: FIX 11 — cancel_all_tp_orders() storniert nur eigene DOM_-Orders.
      Manuell auf Bitget gesetzte TPs ohne ORDER_PREFIX bleiben unberührt.
      cancel_all_tp_orders(dom_only=False) beim Position-Close räumt alles auf.
  F7: FIX 12 — Debounce-Lock in update_tp_for_position().
      _tp_update_lock verhindert parallele Doppel-Aufrufe wenn zwei DCA-Fills
      gleichzeitig ankommen (grosse Limit-Order in mehrere Fills aufgeteilt).
  F8: FIX 3 — Race-Condition: place-first, cancel-after in update_tp_for_position.
      Neue TPs werden ZUERST platziert, dann werden alte storniert. Kein
      Zeitfenster mehr ohne TP-Schutz auf der Exchange.
...
H4_JOCHEN_WINDOW_SEC = _env_int("H4_JOCHEN_WINDOW_SEC", 120)
            ...
            if _sym and _dir:
                ...
                _log_pos_event(_sym, _dir, "TRADE_CLOSE",
                               info=f"Close={close_px} PnL={pnl:+.2f} USDT "
                                    f"ROI={roi_pct:+.1f}% {_won_str} [DEMO]")
        except Exception as e:
...
def _met_timestamp() -> str:
    ...
    try:
        ...
        suffix = "MESZ" if _now.dst() and _now.dst().seconds > 0 else "MEZ"
    ...
    return (datetime.now(timezone.utc) + timedelta(hours=_offset)).strftime("%d.%m.%Y %H:%M MET")
...
def _log_pos_event(symbol: str, direction: str, event: str, **kwargs):
    ...
    Events: TRADE_OPEN, SL_ENTRY, SL_HARSI, SL_SLING, DCA_PLACED, TRADE_CLOSE
    """
...
def cancel_all_tp_orders(symbol: str, dom_only: bool = True):
    ...
    ohne Prefix bleiben unberuehrt. dom_only=False storniert alle (Altverhalten,
    z.B. beim Close einer Position wenn aufgeraeumt werden muss).
...
def calc_tp_price(avg: float, roi: float,
    ...
    """
    Berechnet TP-Preis sodass der NETTO-ROI (nach Entry- und Exit-Gebühren)
    dem gewünschten Ziel entspricht. [FIX 4 v4.36]

    Brutto-ROI = Netto-ROI + 2 x Taker-Fee x Hebel
    Beispiel: TP1 10%, 10x, 0.06% Fee -> fee_adj=1.2% -> Brutto=11.2%
              Trigger = avg * (1 + 0.112/10) statt avg * (1 + 0.10/10)
    """
    fee_adjustment = 2 * TAKER_FEE * leverage   # Entry + Exit Fee auf Kapital
...
def place_tp_orders(symbol: str, avg: float, size: float,
    ...
    for roi, label, pct in partial_tps:
        ...
        # FIX 6 v4.36: Limit-Order mit Buffer statt reiner Market-Order (USE_LIMIT_TP)
        if USE_LIMIT_TP:
            exec_raw = tp_val * (1 - LIMIT_TP_BUFFER) if direction == "long" else tp_val * (1 + LIMIT_TP_BUFFER)
            exec_str = round_price(exec_raw, decimals)
        else:
            exec_str = "0"   # Market-Order bei Trigger
        # FIX 11 v4.36: ClientOid mit ORDER_PREFIX — nur eigene Orders werden storniert
...
def setup_new_trade(pos: dict):
    ...
    _log_pos_event(symbol, direction, "TRADE_OPEN",
                   new_sl=str(round(sl_price, 8)),
                   info=f"Entry={entry:.8g} Lev={leverage}x [DEMO]")
...
def update_tp_for_position(pos: dict, reason: str):
    """
    Wird bei DCA-Fill aufgerufen: setzt TPs auf Basis des neuen Durchschnittspreises.

    v4.36 FIX 12: _tp_update_lock verhindert parallele Doppel-Aufrufe wenn
    zwei DCA-Fills im selben Tick ankommen (z.B. grosse Limit-Order aufgeteilt).

    v4.36 FIX 3: Race-Condition geschlossen — neue TPs werden ZUERST platziert,
    DANN werden die alten storniert. So gibt es kein Zeitfenster ohne TP-Schutz.
    """
    # FIX 12: Debounce — wenn Update bereits laeuft, Doppel-Trigger ignorieren
...
def _update_tp_for_position_impl(pos: dict, reason: str):
    ...
    try:
        _fr_res = api_get("/api/v2/mix/market/current-fund-rate", {
            "symbol": symbol, "productType": PRODUCT_TYPE,
        })
        if _fr_res.get("code") == "00000":
            _rate = float((_fr_res.get("data") or {}).get("fundingRate", 0) or 0)
            _daily_cost = _rate * 3 * leverage * 100   # % auf Kapital/Tag
            if ((direction == "long"  and _rate < FUNDING_WARN_THRESHOLD) or
                ...
                telegram(
                    ...
                    f"Auf Kapital: ~{_daily_cost:.2f}% / Tag bei {leverage}x\n"
                    f"Haltedauer kritisch prüfen!"
                )
                log(f"  FIX5: Funding Rate Warnung {symbol}: {_rate*100:.4f}%/8h")
    except Exception:
        pass
...
def cmd_hilfe():
    reply(
        ...
        "\n"
        "📊 <b>Info &amp; Überblick:</b>\n"
        "/status — Kurzstatus aller offenen Demo-Positionen\n"
        "/pos SYMBOL — Live-Detail: Entry/SL/DCAs/TPs + Event-History\n"
...
def cmd_pos(parts: list):
    """v4.46 [DEMO] — /pos SYMBOL: Live-Detailansicht einer offenen Demo-Position."""
    if len(parts) < 2:
        reply("❌ <b>Verwendung:</b> <code>/pos SYMBOL</code>\nBeispiel: <code>/pos TRXUSDT</code>")
        return
    symbol = parts[1].strip().upper()
    positions = get_all_positions()
    pos = next((p for p in positions if p.get("symbol", "").upper() == symbol), None)
    if not pos:
        reply(f"ℹ️ <b>{symbol}</b> — keine offene Demo-Position.\nTipp: <code>/show {symbol}</code> für History.")
        return
    ...
    if open_ts:
        ...
        d, h = divmod(h, 24)
        duration_str = (f"{d}d {h}h {m}m" if d > 0 else f"{h}h {m}m" if h > 0 else f"{m}m")
    else:
        duration_str = "—"
    ...
    # SL: erst aus Position-Feldern, dann _get_pos_sl_price()
    sl = 0.0
    ...
    elif _sling_px and sl > 0 and abs(_sling_px - sl) / max(sl, 0.00001) < 0.001:
        sl_type = "🏹 Sling-Trail"
    elif _harsi_px and sl > 0 and abs(_harsi_px - sl) / max(sl, 0.00001) < 0.001:
        sl_type = "📉 HARSI-Trail"
    elif sl > 0:
    ...
    if sl > 0:
        ...
        sl_liq_pct    = sl_dist_entry * leverage
        sl_line = (f"  {sl_type}\n"
    ...
    # DCAs: LONG absteigend, SHORT aufsteigend (nächste zuerst)
    dca_orders = get_existing_dca_orders(symbol, direction)
    dca_orders.sort(key=lambda o: float(o.get("price", 0)), reverse=(direction == "long"))
    if dca_orders:
        dca_lines = []
        for i, o in enumerate(dca_orders, 1):
            ...
            dca_lines.append(
                ...
                f"  (≈ {notional:.1f} USDT)\n"
                f"    Entry +{dist_entry:.1f}%  |  Mark +{dist_mark:.1f}%")
        dca_block = "\n".join(dca_lines)
    else:
        dca_block = "  (keine ausstehenden DCA-Orders)"

    # TPs: sortiert nach Auslösereihenfolge
    tps = get_existing_tps(symbol)
    ...
    if tps:
        ...
        for i, tp in enumerate(tps, 1):
            ...
            label  = "TP4 (TP-SL)" if is_tp4 else f"TP{i}"
    ...
    has_open_event = any(len(r) > 3 and r[3].strip() == "TRADE_OPEN" for r in events)
    ev_lines = []
    ...
    reply(
        f"🧪 [DEMO]  {dir_icon} <b>{symbol} {direction.upper()} {leverage}x</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"\n"
        f"📍 <b>Position</b>\n"
...
def cmd_show(parts: list):
    """v4.45 [DEMO] — /show SYMBOL: letzter Trade + Coin-Statistik."""
    if len(parts) < 2:
        reply("❌ <b>Verwendung:</b> <code>/show SYMBOL</code>\nBeispiel: <code>/show TRXUSDT</code>")
        return
    symbol = parts[1].strip().upper()
    rows: list = []
    try:
        ...
        reply(f"❌ CSV-Lesefehler: {e}"); return
    if not rows:
        reply(f"❌ Keine Trades für <b>{symbol}</b> gefunden."); return

    total  = len(rows)
    wins   = sum(1 for r in rows if r[11].strip().lower() in ("ja", "yes", "true", "1"))
    losses = total - wins
    wr     = wins / total if total > 0 else 0.0
    pnls   = [float(r[7].replace(",", ".")) for r in rows if len(r) > 7 and r[7].replace(",","").replace(".","").lstrip("-").isdigit() or (len(r) > 7 and r[7])]
    try:
        pnls = [float(r[7].replace(",", ".")) for r in rows]
    except Exception:
        pnls = []
    try:
        rois = [float(r[8].replace(",", ".")) for r in rows]
    except Exception:
        rois = []
    total_pnl = sum(pnls)
    ...
    dir_icon = "🟢" if last[3].upper() == "LONG" else "🔴"
    filled   = round(wr * 10)
    bar      = "█" * filled + "░" * (10 - filled)
    recent   = rows[-5:]
    trend    = " ".join("✅" if r[11].strip().lower() in ("ja","yes","true","1") else "❌" for r in recent)
    pnl_sign = "+" if total_pnl >= 0 else ""
    apnl_s   = "+" if avg_pnl >= 0 else ""
    aroi_s   = "+" if avg_roi >= 0 else ""

    reply(
        f"🧪 [DEMO]  📊 <b>{symbol} — Trade-Übersicht</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🕐 <b>Letzter Trade</b> — {last[0]}  {last[1]} UTC\n"
        f"{dir_icon} {last[3].upper()} · {last[4]}x · {'✅' if last_won else '❌'}\n"
        f"  Entry:  <code>{last[5]}</code>\n"
        f"  Close:  <code>{last[6]}</code>\n"
        f"  PnL:    <b>{last[7]} USDT</b>  ({last[8]}%)\n"
        f"  Dauer:  {last[9] if len(last) > 9 else '—'}\n\n"
        f"📈 <b>Gesamt — {total} Trades</b>\n"
        f"  Wins / Losses:  {wins} ✅ / {losses} ❌\n"
        f"  Win-Rate:  {wr*100:.1f}%  {bar}\n"
        f"  Ø PnL/Trade:  <b>{apnl_s}{avg_pnl:.2f} USDT</b>\n"
        f"  Ø ROI/Trade:  {aroi_s}{avg_roi:.1f}%\n"
        f"  Gesamt PnL:   <b>{pnl_sign}{total_pnl:.2f} USDT</b>\n"
        f"  Bester Trade:  +{best_pnl:.2f} USDT\n"
        f"  Schlechtester: {worst_pnl:.2f} USDT\n\n"
        f"🔥 <b>Trend (letzte {len(recent)}):</b>  {trend}"
    )


def cmd_coins(parts: list = None):
    """v4.45 [DEMO] — /coins [n]: Ranking aller Coins nach Gesamt-PnL."""
    sym_data: dict = {}
    try:
        if not TRADES_CSV or not os.path.isfile(TRADES_CSV):
            reply(f"❌ trades.csv nicht gefunden: <code>{TRADES_CSV}</code>"); return
        with open(TRADES_CSV, "r", encoding="utf-8") as f:
            reader = csv.reader(f, delimiter=";")
            next(reader, None)
            for row in reader:
                ...
                if sym not in sym_data:
                    sym_data[sym] = {"wins": 0, "n": 0, "pnl": 0.0}
                sym_data[sym]["n"]   += 1
        ...
        reply(f"❌ CSV-Lesefehler: {e}"); return
    if not sym_data:
        reply("❌ Keine Trade-Daten gefunden."); return

    ranked = sorted(sym_data.items(), key=lambda x: x[1]["pnl"], reverse=True)
    total_syms = len(ranked)
    n_show = 5
    if parts and len(parts) >= 2:
        try: n_show = max(1, min(int(parts[1]), 20))
        except ValueError: pass

    lines = [
        f"🧪 [DEMO]  🏆 <b>Coin-Ranking — {total_syms} Coins"
        f" ({sum(v['n'] for v in sym_data.values())} Trades)</b>",
        "━━━━━━━━━━━━━━━━━━━━",
    ]

    def _line(rank, sym, d):
        wr  = d["wins"] / d["n"] if d["n"] > 0 else 0.0
        pnl = d["pnl"]
        sign = "+" if pnl >= 0 else ""
        medal = {1: "🥇", 2: "🥈", 3: "🥉"}.get(rank, f"  {rank}.")
        bar = "█" * round(wr * 5) + "░" * (5 - round(wr * 5))
        return (f"{medal} <b>{sym}</b>  {sign}{pnl:.2f} USDT\n"
                f"    {d['wins']}✅/{d['n']-d['wins']}❌  ({wr*100:.0f}%) {bar}")

    if total_syms <= n_show * 2:
        lines.append("")
        for i, (sym, d) in enumerate(ranked, 1):
            lines.append(_line(i, sym, d))
    else:
        lines.append(f"\n🚀 <b>Top {n_show}</b>")
        for i, (sym, d) in enumerate(ranked[:n_show], 1):
            lines.append(_line(i, sym, d))
        lines.append(f"\n💀 <b>Flop {n_show}</b>")
        for i, (sym, d) in enumerate(ranked[total_syms - n_show:], total_syms - n_show + 1):
            lines.append(_line(i, sym, d))
...
def cmd_trades():
    ...
    Caption enthält Zeilenzahl, Dateigrösse und Mtime beider Dateien. [DEMO]"""
    ...
    if not TRADES_CSV:
        ...
        try:
            ...
            fname   = f"demo_trades_{ts_tag}.csv"
            caption = (
                "📦 <b>[DEMO] Trade-Archiv — trades.csv</b>\n"
                f"━━━━━━━━━━━━\n"
    ...
    if not ENTRY_LOG_CSV:
        ...
        try:
            ...
            fname   = f"demo_entry_queue_log_{ts_tag}.csv"
            caption = (
                "📊 <b>[DEMO] Entry-Queue-Log — entry_queue_log.csv</b>\n"
                f"━━━━━━━━━━━━\n"
...
def _h4_state_update(symbol: str, direction: str, entry: float) -> None:
    """v4.60 Demo: H4-Trigger State setzen/ersetzen (Railway-Gate Modus)."""
    if not H4_GATE_ENABLED:
        return
    key  = f"{symbol}_{direction}"
    prev = h4_trigger_state.get(key, {})
    h4_trigger_state[key] = {"ts": time.time(), "entry": entry, "active": True}
    if prev.get("active"):
        log(f"  [H4-Gate] {key}: H4-Trigger ERSETZT (alter war noch aktiv @ {prev.get('entry', '?'):.5f})")
    else:
        log(f"  [H4-Gate] {key}: H4-Trigger GESETZT @ {entry:.5f}")


def _h4_gate_check(symbol: str, direction: str) -> tuple[bool, str]:
    """v4.60 Demo: H2_SIGNAL gegen H4-Trigger prüfen + Jochen-Regel."""
    if not H4_GATE_ENABLED:
        return True, "H4-Gate deaktiviert"
    key   = f"{symbol}_{direction}"
    state = h4_trigger_state.get(key)
    if not state or not state.get("active"):
        return False, f"Kein aktiver H4-Trigger für {key}"
    age = time.time() - state["ts"]
    if age < H4_JOCHEN_WINDOW_SEC:
        log(f"  [H4-Gate] Jochen-Regel: {key} — H4 vor {age:.0f}s, H2 zu früh. H4 bleibt aktiv.")
        return False, f"Jochen-Regel: H4 vor {age:.0f}s gesetzt (< {H4_JOCHEN_WINDOW_SEC}s)"
    h4_trigger_state[key]["active"] = False
    log(f"  [H4-Gate] {key}: H4-Trigger KONSUMIERT (H4-Alter {age:.0f}s) → Trade-Freigabe!")
    return True, f"H4-Gate OK — konsumiert @ {state['entry']:.5f}"
...
def start_webhook_server():
    ...
    def _webhook_dispatch(data: dict) -> None:
        ...
        if signal_type == "SLING_SL":
            ...
            if _tp_level >= 2:
                log(f"  ⏭ [DEMO] SLING_SL {symbol} ignoriert — TP{_tp_level} erreicht, "
        ...
        if timeframe == "H4" or signal_type == "H4_TRIGGER":
            # v4.60: H4-Gate State Machine (Railway-Gate Modus)
        ...
        # v4.60: H4-Gate Prüfung (Railway-Gate Modus)
        ...
        if _h4_gate_mode == "railway":
            ...
            if not _gate_ok:
                ...
                if "Jochen-Regel" in _gate_reason:
                    telegram(
                        f"⏳ <b>[DEMO] Jochen-Regel</b> — {symbol} {direction.upper()}\n"
                        f"H4+H2 quasi-simultan — H2 verworfen. H4 bleibt aktiv. ✅"
                    )
                return
    ...
    try:
        ...
        for _sym, _snap in (s.get("recent_closes", {}) or {}).items():
            try:
                ...
                pass
        # v4.60: H4-Trigger State Machine laden
        _h4_saved = s.get("h4_trigger_state", {})
        _h4_restored = 0
        for _k, _v in _h4_saved.items():
            try:
                if bool(_v.get("active", False)) and (_now_ts - float(_v.get("ts", 0))) < 8 * 3600:
...
def check_and_repair_position(pos: dict):
    ...
    _initial_size = trade_data.get(symbol, {}).get("initial_size",
                    trade_data.get(symbol, {}).get("init_size", 0))
...
def report_position_startup(pos: dict):
    ...
    trade_data[symbol] = {
        ...
        "peak_size":    size,
        "initial_size": _prev_td_p.get("initial_size", _prev_td_p.get("init_size", size)),  # v4.51: erhalten
        "open_ts":      int(time.time() * 1000),
...
def main():
    ...
    while True:
        ...
        try:
            ...
            # FIX 9 v4.36: fetch_time VOR dem API-Call sichern. Fills die
            # waehrend setup_new_trade/update_tp_for_position eintreffen (3-10s)
            # wuerden sonst beim naechsten Tick nicht mehr erkannt.
            ...
            if open_fills:
                ...
                last_check_ms = fetch_time   # FIX 9: auch im else-Zweig aktualisieren
