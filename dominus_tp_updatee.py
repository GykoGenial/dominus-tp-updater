    ...
    for roi, label, pct in partial_tps:
        ...
        if res.get("code") == "00000":
            ...
            if "checkBDScale" in msg or "checkScale" in msg:
                # Bitget erwartet ganzzahlige Qty für dieses Instrument (checkScale=0)
                # Preis-Dezimalen bleiben, nur Qty wird auf Integer gerundet (floor)
                qty_int = int(qty)
                if qty_int <= 0:
                    log(f"    ✗ {label}: qty {qty} → Integer-Rundung = 0 — übersprungen")
                    partial_tps_skipped += 1
                    continue
                log(f"    ↩ Retry mit integer Qty: {qty} → {qty_int}")
                ...
                if res2.get("code") == "00000":
                    log(f"    ✓ {label} @ {tp_str} USDT [integer qty {qty_int} ✓]")
                    count += 1
                    prices.append(f"{label}: {tp_str}")
...
def flush_h2_buffer():
    ...
    # ── InlineKeyboard ────────────────────────────────────────────────────────
    # ≤ 4 Signale → volle Breite (1 Button/Zeile, langer Label mit Richtung)
    # > 4 Signale → halbe Breite (2 Buttons/Zeile, kurzer Label)
    n         = len(ordered)
    use_2cols = n > 4
    keyboard  = []
    ...
    def _btn(item):
        ...
        prem_flag = "1" if prem else "0"
        if use_2cols:
            label = f"{icon} {short_sym}"           # kompakt für 2-Spalten
        else:
            prem_label = " PREMIUM" if prem else ""
            label = f"{icon} {short_sym}{prem_label} {drct.upper()}"
        return {"text": label, "callback_data": f"h2trade:{sym}:{drct}:{ent}:{prem_flag}"}
    ...
    if use_2cols:
        ...
        for item in ordered:
            ...
            if len(row) == 2:
                ...
                row = []
        if row:
            keyboard.append(row)    # letzter Button allein wenn ungerade Anzahl
    else:
        for item in ordered:
    ...
    telegram_with_buttons("\n".join(lines), keyboard)
    n_prem = len(prem_shorts) + len(prem_longs)
